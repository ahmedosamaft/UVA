...
void solve() {
  ...
  for (int i = 0; i < v.size(); ++i) {
    auto rectangle = v[i];
    pii ul = rectangle[0], lr = rectangle[1];
    double x1 = ul.ff, x2 = lr.ff, y1 = lr.ss, y2 = ul.ss;
    if (point.ff > x1 && point.ff < x2 && point.ss > y1 && point.ss < y2) {
      cout << s << "contained in figure " << i + 1 << endl;
      f = 1;
    }
  }

  if (!f)
    cout << s << "not contained in any figure\n";
}
...
signed main() {
  ...
  int cnt = 0;
  while (cin >> point, point != make_pair(9999.9, 9999.9)) {
    s = "Point " + to_string(++cnt) + " is ";
    solve();
  }
  return 0;
}
